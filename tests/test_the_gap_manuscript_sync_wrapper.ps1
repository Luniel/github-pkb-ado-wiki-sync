...
try {
    ...
    & $Wrapper unsupported -ConfigPath $cfg >$null 2>$null; Assert-True ($LASTEXITCODE -ne 0) 'unsupported action remains nonzero'
    ...
    foreach ($cmd in @('py.cmd','python.cmd','python3.cmd')) { Set-Content -LiteralPath (Join-Path $bin $cmd) -Value '@echo off`r`nexit /b 1' -Encoding ASCII }
    ...
    if ($realPython) {
        Set-Content -LiteralPath (Join-Path $bin 'py.cmd') -Value '@echo off`r`nexit /b 1' -Encoding ASCII
        Set-Content -LiteralPath (Join-Path $bin 'python.cmd') -Value '@echo off`r`nexit /b 1' -Encoding ASCII
        Set-Content -LiteralPath (Join-Path $bin 'python3.cmd') -Value "@echo off`r`n`"$realPython`" %*" -Encoding ASCII
        $r=Invoke-Script $fake.Wrapper 'status' $fakeCfg @{ PATH=$bin }; Assert-True ($r.Code -eq 0 -and $r.Out -match 'stub stdout') 'unusable earlier candidates fall back to next usable Python 3'

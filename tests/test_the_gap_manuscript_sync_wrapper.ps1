...
try {
    ...
    $badOut = Join-Path $TempRoot 'unsupported.out'; $badErr = Join-Path $TempRoot 'unsupported.err'; $childShell = if ($PSVersionTable.PSEdition -eq 'Core') { 'pwsh.exe' } else { 'powershell.exe' }; $child = Start-Process -FilePath $childShell -ArgumentList @('-NoProfile','-NonInteractive','-File',$Wrapper,'unsupported','-ConfigPath',$cfg) -RedirectStandardOutput $badOut -RedirectStandardError $badErr -Wait -PassThru; $badText = (Get-Content -LiteralPath $badErr -Raw -ErrorAction SilentlyContinue); Assert-True ($child.ExitCode -ne 0 -and $badText -match 'Action|ValidateSet|parameter') 'unsupported action child process remains nonzero and reports parameter validation'
